...
async function Rank(msg, args) {
    ...
    const platform = args && args.length > 0 ? await args.shift().toLowerCase() : "xbox";
    if(!validPlatforms.find(pf => pf === platform)) return await msg.channel.send(`That is not a valid platform! Choose from: ${validPlatforms.join(", ")}`);
    let userId = await args.join("-");
    
    userId = !userId ? "Ruffect"
    ...
    switch(platform) {
        ...
        case "xbox":
            profileData = {
                ...
                avatarUrl: `./src/files/rocket_league/${userId == "Ruffect" ? "ruffect_avatar" : "xbox"}.png`
            };
            break;
        default:
            break;
...
async function sendRankData({ msg, rankData, profileData }) {
    ...
    const imageAttachment = new Discord.MessageAttachment(imageBuffer, "rank.png");
        ...
        for (let j = 0; j < selectedRow.length; j++) {
            ...
            if(elementData && !isNaN(elementData.rank)) {
                if(!rankImages[elementData.rank]) {
                    rankImages[elementData.rank] = await Canvas.loadImage(`./src/files/rocket_league/rank_icons/${elementData.rank}.png`);
                }
